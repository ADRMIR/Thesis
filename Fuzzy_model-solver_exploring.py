...
avg_result=np.empty([0,10])
avg_result_fuzzy=np.empty([0,10])
avg_result_fuzzy_FE=np.empty([0,10])

avg_avg_result_fuzzy_FE=np.empty([0,10])

maximum_range=50

maximum_number_of_neurons=30

solvers=['lbfgs','adam','sgd']

t1=time()

j=4     #Number of hidden neurons

for s in solvers:   #Goes through the different algorithm solvers
    ...
    for i in range(maximum_range):
        ...
        hidden=(j)
        clf_fuzzy_with_FE=MLPCla(hidden_layer_sizes=(hidden),activation='logistic', solver=s,alpha=1e-5,max_iter=5000,early_stopping=True,verbose=0).fit(X_train_fuzzy_FE,y_train_fuzzy_FE)
    ...
    print()
    # print( round((time() - t0)/60), " minutes to train ", maximum_range,"  models with ", j+1, "hidden layers")
    print( round((time() - t0)/60), " minutes to train ", maximum_range,"  models with (", j, ") Neurons")
    print()
    
    avg_results_to_pass=np.hstack((np.mean(result_fuzzy_FE,axis=0)))
    avg_result_fuzzy_FE=np.vstack((avg_result_fuzzy_FE,avg_results_to_pass))
...
plt.scatter(solvers,avg_result_fuzzy_FE[:,0],label="Accuracy (%)")
plt.scatter(solvers,avg_result_fuzzy_FE[:,1],label="F-Score")
plt.scatter(solvers,avg_result_fuzzy_FE[:,2],label="MCC")
plt.scatter(solvers,avg_result_fuzzy_FE[:,3],label="ROC AUC")
plt.scatter(solvers,avg_result_fuzzy_FE[:,4],label="Cohen's kappa")
plt.legend()
plt.title("Evolution of metrics with increase in hidden neurons")
plt.xlabel('Solver')
plt.ylabel("Metrics")
plt.show()